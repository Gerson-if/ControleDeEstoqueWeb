...
function addProduct() {
  ...
  const newProductQuantity = parseInt(
    document.getElementById("newProductQuantity").value,
    10
  );
  const newProductImageURL =
...
function loadEditFormList() {
  ...
  fetch("http://localhost/bonito/database.php")
    .then((response) => response.json())
    .then((data) => {
...
function editProduct() {
  ...
  const editProductQuantity = parseInt(
    document.getElementById("editProductQuantity").value,
    10
  );
  const editProductImageURL = document.getElementById(
    "editProductImageURL"
  ).value;
  ...
  if (
    productList.some(
      (product) => product.id !== productId && product.nome === editProductName
    )
  ) {
    Swal.fire({
      icon: "error",
...
function loadRemoveFormList() {
  ...
  fetch("http://localhost/bonito/database.php") // Substitua a URL pela correta
    .then((response) => response.json())
    .then((data) => {
      data.forEach((product) => {
        const option = document.createElement("option");
...
function removeProduct() {
  const selectedProducts = Array.from(
    document.getElementById("productListRemove").selectedOptions
  );
  ...
  const productIdsToRemove = selectedProducts.map((option) =>
...
function updateProductQuantity(product) {
  const quantityDisplay = document.querySelector(
    `[data-product-id="${product.id}"] .card-text`
  );
  ...
  fetch(apiUrl, {
    ...
    .then((data) => {
      if (data.success) {
        ...
        console.error(
          "Erro ao atualizar a quantidade do produto no banco de dados."
        );
      }
    })
    .catch((error) =>
