...
if ($_SERVER["REQUEST_METHOD"] === "GET") {
    ...
    $sql = "INSERT INTO aquidauana (nome, quantidade, imagem) VALUES ('" . $data["nome"] . "', " . $data["quantidade"] . ", '" . $data["imagem"] . "')";
    ...
    $sql = "UPDATE aquidauana SET nome='" . $data["nome"] . "', quantidade=" . $data["quantidade"] . ", imagem='" . $data["imagem"] . "' WHERE id=" . $data["id"];
    if ($conn->query($sql) === TRUE) {
    ...
    $sql = "DELETE FROM aquidauana WHERE id IN ($ids)";
    if ($conn->query($sql) === TRUE) {
        echo json_encode(array("success" => true));
    } else {
        echo json_encode(array("success" => false));
